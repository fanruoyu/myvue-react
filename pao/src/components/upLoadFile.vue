  ...
  <div id="businessUpLoadFile">
    <div class="componentBox">
      <div class="titleComponentName">
        ...
        <el-upload
          :style="{display: disabled?'none':'', float:bLight ? 'left':''}"
          class="upload-demo"
          :action="url"
          :data="fileData5"
          :headers="headers"
          ...
          :limit="1">
          <el-button v-if="bLight" size="small" type="primary">上传</el-button>
...
<script>
  export default {
    name: 'upLoadFile',
    props: ['title', 'disabled', 'fieldName', 'fileObj', 'bLight'],
    data() {
      return {
    ...
    methods: {
      ...
      success(response, file, fileList) {
        ...
        for (var i = 0; i < response.result.length; i++) {
          ...
          this.fileObj[this.fieldName].unshift(response.result[i])
        }
      },
      error() {
        this.$emit('switchLoading', false)
