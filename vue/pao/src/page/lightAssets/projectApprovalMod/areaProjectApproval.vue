    ...
    <div id="conModd" :class='{overflow:show1==true}'>
      <div class="newProject">
                ...
                <tr style="height:40px;">
                 ...
                 <tr class="sec-tr" v-if="marst" v-for="(item,index) in messageList">
                    ...
                    <td><span class="examine"  v-if="messageList[index].approvalStatus== 7 || messageList[index].approvalStatus== 6 || messageList[index].approvalStatus== 4 || messageList[index].approvalStatus== 2" ><router-link style="width:100%;height:100%;" :to="{path:'/heavyDetailEntry',query: {id:messageList[index].id,projectName:messageList[index].projectName,auth:1}}">查看</router-link></span><span class="examine" v-if="messageList[index].approvalStatus== 8 || messageList[index].approvalStatus== 5 || messageList[index].approvalStatus== 1 || messageList[index].approvalStatus== 0 || messageList[index].approvalStatus== 3"><router-link style="width:100%;height:100%;" :to="{path:'/heavyDetailEntry',query: {id:messageList[index].id,projectName:messageList[index].projectName,auth:2}}">修改</router-link></span>
                       <span class="repulse"   v-if="messageList[index].contractStatus!=1&& messageList[index].contractStatus!=2 && messageList[index].contractStatus!=0 ">提交审批</span>
                       <span class="repulse"   v-if="messageList[index].contractStatus==1" @click="signDetails(index)" >签约详情</span>
                       <span class="repulse"   v-if="messageList[index].contractStatus!=2 && messageList[index].contractStatus!=1 " @click="popUp(index)">签约</span>
       ...
       <div v-if="pop" style="width:100%;height:100%;background-color: rgba(0, 0, 0, 0.3);position: fixed;top: 0;left: 0;z-index:2000;" v-loading="loadingA"
          ...
          <div class="suspension" v-if="shade">
            <span style="margin-left:10px;">签约</span><span style="float:right;margin-right:20px;cursor:pointer;" @click="abolish">X</span>
             <hr>
            <input type="radio" name="radiobutton" value="signingOn"  style="margin-left:10px;" @click="signingOn "  checked><span>成功签约</span>
            <input type="radio" name="radiobutton" value="runOff"  style="margin-left:100px;" @click="erosion"><span>流失</span>
            <div v-if="isPopp"> 
                <el-upload style="width:100px;;" :action='url'
                           multiple :limit="1"  :data="fileData5" :headers="headers"
                           :show-file-list='false' :before-upload="beforeAvatarUpload" :on-success="successContract" :on-progress="upLoad" >
                <span size="small" class="clickUpa" type="primary">上传合同</span>
                </el-upload>
                <table style="width:60%;margin-left:10px;">
                    <tr v-for="(item,index) in contract" >
                      ...
                      <td ><p style="height:80%;margin-top:10%;"><span class="delete" @click="handleRemoveD(index)">删除</span></p></td>
                    </tr>
                </table>
            </div>
           <div class="textareaBox" v-if="loss">
            <textarea name="" type="textarea" rows="2" autocomplete="off" v-model='considerationText'  maxlength="300" ref="considerationText"
              class="decision" placeholder="请输入" validateevent="true" ></textarea>
            <span class="statistics">{{considerationText.length}}/300</span>
            <span v-if="considerationText.length==300" style="color:red;margin-left:10px;">文字个数已达上限</span>
          </div>
            <hr style="color:#DDDDDD;">
            <p><span class="left"  @click="subMiter()">保存</span><span class="right" @click="abolish">取消</span></p>
       </div>
...
<script>
  ...
  capitalize:function(value){
    ...
    methods:{
      ...
      //上传合同的钩子函数
      successContract(response, file, fileList) {
        for (var i = 0; i < response.result.length; i++) {
          ...
          this.contract.push({'fileName':response.result[i].fileName, "filePath":"",'fileUrl':response.result[i].fileUrl})
        }
          this.loadingA = false,
          this.subMit   = true
      },
    ...
    //分页索引减少
      ...
      erosion(){
       ...
       if(this.isPopp){
        ...
        if(this.loss){
          if(this.considerationText.length==0){
            return
          }else{
           this.subText();
          }
      ...
      popUp(index){
       ...
       subList() {
         ...
         this.$http({
          method: 'post',
          url: '/longcity/services/light/v1/approve/lightProject/saveLoseOrConvention',
          data: {
                "filesList"   : this.contract,
                "lightProjectId" : this.messageList[this.indexs].id,
                "status"     : '1'
                }
        }).then(function (succeed) {
          if (succeed.data.code == 200 || succeed.data.code == "200") {
            console.log("保存成功")
             that.shade = false;
             that.pop = false;
             that.messageList[that.indexs].contractStatus = 1;
             that.contract = [];
            }
        }, function (err) {
           that.shade = false;
       ...
       subText() {
         ...
         this.$http({
          ...
          url: '/longcity/services/light/v1/approve/lightProject/saveLoseOrConvention',
          data: {
                "loseCase"   :  this.considerationText,
                "status"     :  '2',
                 "lightProjectId" : this.messageList[this.indexs].id
                }
        }).then(function (succeed) {
          if (succeed.data.code == 200 || succeed.data.code == "200") {
             that.shade = false;
             that.pop = false;
             that.messageList[that.indexs].contractStatus = 2;
             that.considerationText = ''
          }
        }, function (err) {
          console.log(err);
        })
